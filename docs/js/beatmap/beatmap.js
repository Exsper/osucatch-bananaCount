...
function Beatmap(osu, mods)
{
    ...
    while (typeof (line = stream.pop()) != 'undefined')
    {
        ...
        switch (currentSection)
        {
            ...
            case 'Events':
            {
                var data = line.split(',');
                if (data[0] == "2") {
                    this.Breaks.push({start: parseInt(data[1]), end: parseInt(data[2])});
                }
                break;
            }
            case 'TimingPoints':
    ...
    this.OriginHPDrainRate = this.HPDrainRate;
    this.OriginCircleSize = this.CircleSize;
    this.OriginOverallDifficulty = this.OverallDifficulty;
